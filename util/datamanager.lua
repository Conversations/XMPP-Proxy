...
function load(username, host, datastore)
	...
	if not data then
		...
		if not mode then
			log("debug", "Assuming empty %s storage ('"..ret.."') for user: "..(username or "nil").."@"..(host or "nil"), datastore);
			return nil;
		else -- file exists, but can't be read
			-- TODO more detailed error checking and logging?
			log("error", "Failed to load %s storage ('"..ret.."') for user: "..(username or "nil").."@"..(host or "nil"), datastore);
	...
	if not success then
		log("error", "Unable to load %s storage ('"..ret.."') for user: "..(username or "nil").."@"..(host or "nil"), datastore);
		return nil, "Error reading storage";
	end
	return ret;
end
...
function store(username, host, datastore, data)
	...
	if not f then
		log("error", "Unable to write to %s storage ('"..msg.."') for user: "..(username or "nil").."@"..(host or "nil"), datastore);
